 ...
 */
export function getDefaultParams(args) {
    return {
 ...
 * 获取应用列表（带缓存）
 */
export async function getAppList(appKey) {
    const now = Date.now();
    if (appListCache && (now - appListCacheTime) < CACHE_DURATION) {
        return appListCache;
    }
    try {
        const response = await axios.post(`${process.env.JIANDAOYUN_BASE_URL || 'https://api.jiandaoyun.com'}/api/v5/app/list`, {}, {
            headers: {
 ...
 */
export async function resolveFormId(inputId, appKey) {
    ...
    if (targetApp) {
        ...
        try {
            const response = await axios.post(`${process.env.JIANDAOYUN_BASE_URL || 'https://api.jiandaoyun.com'}/api/v1/app/${inputId}/entry/list`, {}, {
                headers: {
                    'Authorization': `Bearer ${appKey}`,
                    'Content-Type': 'application/json'
                }
 ...
 */
export async function smartFieldMapping(formId, userData, appKey, appId) {
    try {
        // 获取表单字段信息
        const response = await axios.post(`${process.env.JIANDAOYUN_BASE_URL || 'https://api.jiandaoyun.com'}/api/v5/app/entry/widget/list`, {
            app_id: appId,
            entry_id: formId
        }, {
            headers: {
