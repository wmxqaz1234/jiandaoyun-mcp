...
import { resolveFormId, smartFieldMapping, getDefaultParams } from '../../utils/index.js';
/**
 * 创建新的 MCP 服务器实例
 */
export function createMcpServer(options) {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        function generateTable(data) {
            ...
            // 统计时间范围
            const timeFields = Object.keys(data[0] || {}).filter(key => key.toLowerCase().includes('time') || key.toLowerCase().includes('date'));
        ...
        try {
            switch (normalizedToolName) {
                ...
                case 'submit_form_data': {
                    const { formId, data, autoMatch = true, transactionId } = args;
                    const { appId, appKey, baseUrl } = getDefaultParams(args);
                    // 验证必需参数
                    if (!appKey) {
                        ...
                        const formatDataForSubmission = (data) => {
                            ...
                            for (const [key, value] of Object.entries(data)) {
                                ...
                                if (Array.isArray(value) && value.length > 0 && typeof value[0] === 'object') {
                                    formatted[key] = {
                                        value: value.map((item) => formatDataForSubmission(item))
                                    };
                                    continue;
                                }
                                // 处理复杂对象（如地址、定位等）
                ...
                case 'query_form_data': {
                    const { formId, dataId, fields, filter, limit } = args;
                    const { appId, appKey, baseUrl } = getDefaultParams(args);
                    // 验证必需参数
                    if (!appKey) {
                        return {
                ...
                case 'update_form_data': {
                    const { formId, dataId, data, transactionId, isStartTrigger } = args;
                    const { appId, appKey, baseUrl } = getDefaultParams(args);
                    // 验证必需参数
                    if (!appKey) {
                        return {
                ...
                case 'delete_form_data': {
                    const { formId, dataIds, isStartTrigger } = args;
                    const { appId, appKey, baseUrl } = getDefaultParams(args);
                    // 验证必需参数
                    if (!appKey) {
                        return {
                ...
                case 'get_upload_token': {
                    const { formId, transactionId, fieldId } = args;
                    const { appId, appKey, baseUrl } = getDefaultParams(args);
                    // 验证必需参数
                    if (!appKey) {
                        return {
                ...
                case 'list_apps_and_forms': {
                    const { appId: specificAppId } = args;
                    const { appId, appKey, baseUrl } = getDefaultParams(args);
                    // 验证必需参数
                    if (!appKey) {
                        return {
